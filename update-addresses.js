...
function updateContractAddresses() {
  ...
  let deploymentInfo;
  try {
    deploymentInfo = JSON.parse(fs.readFileSync('Mantle-deployment.json', 'utf8'));
  } catch (error) {
    console.error('❌ Mantle-deployment.json not found. Please deploy contracts first.');
    console.log('Run: npm run deploy:Mantle-simple');
    return;
  }
  ...
  readmeContent = readmeContent.replace(
    ...
    `PropertyNFT:    ${PropertyNFT}  # Deployed on Mantle testnet`
  );

  readmeContent = readmeContent.replace(
    /Fractionalizer:\s+0x0000000000000000000000000000000000000000\s+# Update after deployment/g,
    `Fractionalizer: ${Fractionalizer}  # Deployed on Mantle testnet`
  );
  ...
  console.log(`PropertyNFT: https://testnet-explorer.Mantle.network/address/${PropertyNFT}`);
  console.log(`Fractionalizer: https://testnet-explorer.Mantle.network/address/${Fractionalizer}`);

  console.log('\n📝 Next Steps:');
  console.log('1. Test the updated frontend: npm run dev');
  console.log('2. Verify contract interactions work correctly');
  console.log('3. Deploy to Mantle mainnet when ready');
}
